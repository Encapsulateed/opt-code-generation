...
llvm::Value *IfExprAST::codegen() {
    ...
    llvm::Value *ThenVar = Builder.CreateAlloca(llvm::Type::getInt32Ty(MainContext), 0, "thenVar");
    Builder.CreateStore(ThenV, ThenVar);
    ...
    llvm::Value *ElseVar = Builder.CreateAlloca(llvm::Type::getInt32Ty(MainContext), 0, "elseVar");
    Builder.CreateStore(ElseV, ElseVar);
    Builder.CreateBr(MergeBB);

    MainFunction->getBasicBlockList().push_back(MergeBB);
    Builder.SetInsertPoint(MergeBB);

    llvm::Value *ThenLoaded = Builder.CreateLoad(ThenVar);
    llvm::Value *ElseLoaded = Builder.CreateLoad(ElseVar);

    llvm::Value *Result = Builder.CreateSelect(CondV, ThenLoaded, ElseLoaded, "iftmp");
    return Result;
}
...
llvm::Value *WhileExprAST::codegen() {
    ...
    llvm::BasicBlock *CondBB = llvm::BasicBlock::Create(MainContext, "cond", MainFunction);
    llvm::BasicBlock *BodyBB = llvm::BasicBlock::Create(MainContext, "whilebody");
    ...
    MainFunction->getBasicBlockList().push_back(BodyBB);
    ...
    return llvm::Constant::getNullValue(llvm::Type::getInt32Ty(MainContext));
}
